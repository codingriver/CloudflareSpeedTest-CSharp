...
{
    private const int ColNo = 4, ColIp = 16, ColLoss = 6, ColDelay = 10, ColJitter = 8, ColSpeed = 14, ColColo = 6, ColRegion = 8;
    ...
    public static void PrintToConsole(IReadOnlyList<IPInfo> results, int maxRows = 10)
    {
        ...
        Console.WriteLine();
        var header = Pad("序号", ColNo) + ColGap + Pad("IP 地址", ColIp) + ColGap + Pad("丢包率", ColLoss) + ColGap + Pad("延迟", ColDelay) + ColGap + Pad("抖动", ColJitter) + ColGap + Pad("下载速度", ColSpeed) + ColGap + Pad("地区码", ColColo) + ColGap + Pad("地区", ColRegion);
        ...
        for (var i = 0; i < take; i++)
        {
            ...
            var delay = r.MinDelayMs != r.MaxDelayMs ? $"{r.MinDelayMs:F0}-{r.MaxDelayMs:F0} ms" : $"{r.DelayMs:F0} ms";
            var jitter = r.JitterMs > 0 ? $"{r.JitterMs:F1} ms" : "-";
    ...
    public static async Task ExportCsvAsync(IReadOnlyList<IPInfo> results, string path, CancellationToken ct = default)
    {
        ...
        var lines = new List<string> { "IP,丢包率,平均延迟(ms),最小延迟(ms),最大延迟(ms),抖动(ms),下载速度(Mbps),地区码,地区" };
        foreach (var r in results)
        {
            var colo = string.IsNullOrEmpty(r.Colo) ? "N/A" : r.Colo;
            var coloZh = ColoProvider.GetColoNameZh(r.Colo);
            var minDelay = r.MinDelayMs == double.MaxValue ? 0 : r.MinDelayMs;
            lines.Add($"{r.IP},{r.LossRate:P2},{r.DelayMs:F0},{minDelay:F0},{r.MaxDelayMs:F0},{r.JitterMs:F1},{r.DownloadSpeedMbps:F2},{colo},{coloZh}");
        }
